...
## 왕복 스퍼 탐지 구멍 수정 — 중간점 1개짜리 직선 스퍼 (2026-07-07)

이전 스퍼 제거 개선(위 섹션) 배포 후에도 사용자가 **새 빌드에서 한 획으로 새로 그린 경로**에
곧은 왕복 스텁이 남는 것을 실기기 스크린샷으로 재확인했다. 저장 경로 재생·구 빌드·스트로크
접합부 등 다른 요인은 문답으로 배제되어 알고리즘 자체의 탐지 구멍으로 확정됐다.

### 원인
`removeSpurs()`의 탐지 루프에 `j >= i + 3`(중간점 2개 이상 요구) 가드가 있었다. 원래 인접점
오탐 방지용이었으나, T-Map이 만드는 왕복 스퍼의 최빈 형태는 **`[진입점, 팁, 복귀점]` 3점
구조(중간점 1개)**다. 이 형태는 복귀가 `j = i + 2`에서 감지되는데 가드가 이를 걸러내
200m 한도 상향·corridor 검사·그린 선 반전 판별에 도달하기도 전에 통과해버렸다. 스크린샷의
스텁이 꺾임 없는 완전한 직선인 것과 정확히 부합했고, 기존 단위 테스트는 중간점 5개짜리
스퍼만 검증해 이 케이스를 놓쳤다.

### 수정 (`SnapToRoadUseCase.kt`)
- 탐지 가드 완화: `j >= i + 3` → `j >= i + 2`. 최종 게이트는 여전히 corridor 폭 검사와
  그린 선 반전 판별이므로, 후보를 폭넓게 잡아도 실제 아티팩트만 정확히 걸러진다.
- 진단 로그 추가: 스퍼 후보가 발견될 때만 `Timber.d`로 anchor 인덱스·왕복 이동거리·중간점
  수·최종 결정과 사유(고리/그린 선 반전/웨이포인트 근접/아티팩트)를 기록. 다음 재현 시
  logcat만으로 어느 판별 단계에서 보존·제거됐는지 바로 진단 가능.

### 검증
- 신규 `SnapToRoadUseCaseTest` 2건 추가 — 중간점 1개짜리 직선 스퍼 제거
  (`straightSinglePointSpurIsRemoved`), 중간점 1개짜리 의도된 왕복 보존
  (`intendedSingleVertexRetraceIsKept`). 기존 2건과 합쳐 총 4건.
- `:app:compileDebugKotlin testDebugUnitTest` BUILD SUCCESSFUL (결과 XML로 4건 통과 확인).
- 실기기 확인 필요: 같은 지역에 같은 그림을 다시 그려 스텁 소멸 여부. 재현 시 logcat의
  진단 로그를 수집하면 손떨림 급꺾임 오보호 케이스 여부까지 확정할 수 있다.

### 범위 외 (의도적 보류)
- 손떨림 급꺾임으로 인한 오보호 강화 — 진단 로그로 실데이터 확보 후 판단.
- 저장된 옛 경로 불러오기 시 despur 후처리, 이어 그리기 접합부 despur 미적용 문제는 별건.
